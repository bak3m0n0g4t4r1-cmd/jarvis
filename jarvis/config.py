...
# === Перебивание речи (barge-in): мгновенно оборвать озвучку, когда пользователь заговорил ===
# Мастер-выключатель: PTT (правый Ctrl) во время речи — мгновенный надёжный обрыв (физический
# сигнал, без эха). voice — обрыв по ТОЧНОМУ «Джарвис…» поверх речи (консервативно: без AEC
# микрофон слышит сам голос Джарвиса, поэтому требуем точный wake + порог по громкости).
BARGE_ENABLED = _get("barge_in", "enabled", True, "JARVIS_BARGE_ENABLED")
BARGE_VOICE_ENABLED = _get("barge_in", "voice_enabled", True, "JARVIS_BARGE_VOICE_ENABLED")
# Энерго-гейт голосового barge (RMS 0..1): сегмент тише — считаем эхом Джарвиса/фоном, не прерываем.
BARGE_MIN_RMS = _get("barge_in", "min_rms", 0.03, "JARVIS_BARGE_MIN_RMS")
...
# Плейсхолдер {минуты} в offer/remind подставляется кодом (длительность перерыва словами,
# напр. «пять минут»). Стиль: лаконично, по-джарвисовски, чуть заботливо, без морализаторства.
_DEFAULT_OFFER_PHRASES = [
    ...
    "Не сочтите за назойливость, сэр: {минуты} в стороне от экрана — и вы свежее. "
    "Вы сами так распорядились.",
...
# Повтор-напоминание (после игнора + затемнения) — чуть настойчивее, но всё так же мягко.
_DEFAULT_REMIND_PHRASES = [
...
# Длительность цикла активности (МИНУТЫ) — случайная ЦЕЛОЕ число минут в диапазоне (секунды 00).
BREAK_CYCLE_MIN_MINUTES = _get("break_reminders", "cycle_min_minutes", 23, "JARVIS_BREAK_CYCLE_MIN")
BREAK_CYCLE_MAX_MINUTES = _get("break_reminders", "cycle_max_minutes", 27, "JARVIS_BREAK_CYCLE_MAX")
...
# Длительность ЗАСЧИТАННОГО перерыва (МИНУТЫ) — случайная целое в диапазоне. Озвучивается в
# предложении ({минуты}) и ею же кредитуется реальный простой. После перерыва — похвала + цикл заново.
BREAK_BREAK_MIN_MINUTES = _get("break_reminders", "break_min_minutes", 4, "JARVIS_BREAK_MIN")
BREAK_BREAK_MAX_MINUTES = _get("break_reminders", "break_max_minutes", 6, "JARVIS_BREAK_MAX")
# Задержка повторного напоминания при игноре (МИНУТЫ) — случайная целое. Считается по АКТИВНОМУ времени.
BREAK_REMIND_MIN_MINUTES = _get("break_reminders", "remind_after_min_minutes", 5, "JARVIS_BREAK_REMIND_MIN")
BREAK_REMIND_MAX_MINUTES = _get("break_reminders", "remind_after_max_minutes", 6, "JARVIS_BREAK_REMIND_MAX")
...
# Период переэнумерации устройств ввода /dev/input (с) — для hotplug. На КОРРЕКТНОСТЬ idle больше
# НЕ влияет (рескан добавляет/убирает устройства, _last_input не трогает — см. грабли).
...
# Минимум отработанного активного времени (мин) для похвалы: хвалим, только если перед перерывом
# была заметная работа ИЛИ перерыв был предложен (иначе «перерыв» во время фильма со случайным
# касанием мыши хвалился бы невпопад). Дефолт 10 мин.
BREAK_PRAISE_MIN_WORK = _get("break_reminders", "praise_min_work_minutes", 10, "JARVIS_BREAK_PRAISE_MIN") * 60
...
BREAK_REMIND_PHRASES = _get("break_reminders", "remind_phrases", _DEFAULT_REMIND_PHRASES, None)
