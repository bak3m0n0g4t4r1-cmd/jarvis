...
def parse_time(text: str):
    ...
    try:
        ...
        if m:
            sub = _BEZ[m.group(1)]
            nxt = _ONES.get(m.group(2)) or _ORD_GEN.get(m.group(2))
            if nxt:
                return (_apply_period((nxt - 1) % 24, period), 60 - sub)
...
_SCHEDULE_HEADER = (
    "# ═══════════════════════════════════════════════════════════════════════════════\n"
    "#  РАСПИСАНИЕ «ДЖАРВИСА» — будильники (в будущем сюда же лягут таймеры/напоминания)\n"
    "# ═══════════════════════════════════════════════════════════════════════════════\n"
    "#  Правится и голосом, и вручную. Поля каждого будильника:\n"
    "#    тип:    утренний | обычный\n"
    "#    время:  \"ЧЧ:ММ\" (24-часовой формат, напр. \"07:00\", \"19:30\")\n"
    "#    метка:  текст или пусто (чтобы различать обычные будильники)\n"
    "#    повтор: ежедневный | разовый\n"
    "#    активен: true | false\n"
    "#    id:     служебный идентификатор — НЕ меняйте\n"
    "#\n"
    "#  ⚠ При изменении будильников ГОЛОСОМ файл перезаписывается — ручные комментарии\n"
    "#    внутри не сохранятся (значения — сохранятся). Структура — задел под будущее.\n"
    "# ═══════════════════════════════════════════════════════════════════════════════\n\n"
)


def read_schedule() -> dict:
    """Прочитать schedule.yaml → {'будильники': [...]}. Нет файла/битый → пустое (не падаем)."""
    try:
        ...
        alarms = data.get("будильники")
        if not isinstance(alarms, list):
            alarms = []
        data["будильники"] = alarms
        return data
    except Exception:
...
def write_schedule(data: dict) -> bool:
    ...
    try:
        path = config.SCHEDULE_FILE
        payload = {"будильники": data.get("будильники", [])}
        for extra in ("таймеры", "напоминания"):  # сохраняем будущие секции, если появятся
            if extra in data:
